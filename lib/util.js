...
exports.globalCreate = function(listResourceURI, objType, requiredFields, optionalFields) {
	...
	return function() {
		...
		cli._restAPI('POST', this._getResourceURI() + "/" + listResourceURI, data, function(err, res) {
			if(err) return cb(err);
			cb(null, new objType(cli, res.body.Sid).load(res.body) );
		});
	}
}
...
exports.globalList = function(listResourceURI, objType) {
	...
	return function(filters, cb) {
		...
		var li = new ListIterator(this._client, this._getResourceURI() + "/" + listResourceURI, objType, filters);
		li.load(cb);
	}
}
...
exports.globalGet = function(objType) {
	...
	return function(Sid, cb) {
		var obj = new objType(theCaller, Sid);
		if(cb) obj.load(cb);
		return obj;
	};
}
...
exports.globalSave = function(cb) {
	...
	obj._client._restAPI('PUT', command, data, function(err, res) {
		if(err) {
			if(cb) cb(err);
			return;
		}
